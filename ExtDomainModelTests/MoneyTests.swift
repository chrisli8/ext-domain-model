...
// MARK: New Unit Tests
    ...
    func testProtocol() {
        // print("TEST CASE: \(tenUSD.description)")
        XCTAssert(tenUSD.description == "USD10.0")
    }
    
    
    // let fiveGBP = Money(amount: 5, currency: "GBP")
    func testProtocolAfterAdd() {
        let total = fiveGBP.add(fiveGBP)
        XCTAssert(total.description == "GBP10.0")
    }
    
    func testMathmaticsProtocol() {
        XCTAssert(tenUSD.add(twelveUSD).amount == 22)
        XCTAssert(twelveUSD.subtract(tenUSD).amount == 2)
    }
    
    func testDoubleExtension() {
        let amount: Double = 10.0
        XCTAssert(amount.USD.amount == 10)
        XCTAssert(amount.USD.currency == "USD")
        XCTAssert(amount.CAN.currency == "CAN")
        XCTAssert(amount.GBP.currency == "GBP")
        XCTAssert(amount.EUR.currency == "EUR")
    }
}
